# Warden launch film — read this first

This is the launch video for Warden. The film's state and every decision already taken live in `STORYBOARD.md` (frame by frame, with a dated "Changes from" log of what was tried and cut) and `BRIEF.md`; the design spec is `frame.md`. Read the log before proposing a change, and see `README.md` for how to verify one. Everything below is the generic HyperFrames guidance.
